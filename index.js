...
app.event('app_mention', async ({ event, say }) => {
  ...
  if (message.match(/skype/)) {
    response.push(`Do you need help recording a Skype interview or importing one you've recorded into a Premiere Project? ^buttonsSlack(Recording Skype Interview, Importing Skype Interview)`)
  }
  ...
  if (message.match(/add skype interview|find skype interview|find studio clip|add studio recording|import video from FORK|import skype|importing skype interview|import video from fork|import video from Fork|import from fork|import from Fork|import from FORK/)) {
    response.push(` If you're trying import a video you recorded in one of our studios or add a Skype interview you recorded with Tom, you can use the Primestream panel. It's basically the same process you use to export a finished video in reverse. In your Premiere Project, Go to Window>Extensions>Primestream Adobe Panel. Login (user is atvn1, there is no password), and search for your video in this field.
    ...
    image("https://i.imgur.com/iRcxVrF.jpg")`)
  }
  ...
  if (message.match(/help/)) {
    response.push(`How can I help you? ^buttonsSlack(Video, Audio, Social, Other Media Center Tech)`)
  }
  ...
  if (message.match(/who is [*] today?|who is producer|who is the editor|ATVN producer|ARN producer)/)) {
    response.push(`^link("https://docs.google.com/spreadsheets/d/1583hUjAFtTHDwVvYIxYPs9w6uZ0NQuQmSNPVl4NkVu4/edit?usp=sharing","Here's a Google Doc listing each day's student editors, directors and producers.")`)
  }
  if (message.match(/video/)) {
    response.push(`What are you working on for video? ^buttonsSlack(Video Editing, Video Exporting)`)
  }
  ...
  if (message.match(/social|social video/)) {
    response.push(`The URL for downloading all our social templates is: http://bit.ly/annenbergtemplates Here's a ^link("http://resources.uscannenbergmedia.com/wp-content/uploads/2017/10/SocialVideoGuide-1.png","handy one-page guide to doing social video.")`)
  }
  if (message.match(/audio|sound|volume|gain|channels|loud|quiet/)) {
    response.push(`What kind of audio are you working with? ^buttonsSlack(Audio in a Video, Standalone Audio)`)
  }
  if (message.match(/audio in a video/)) {
    response.push(`Use audio normalization (hotkey="g") to make your segment's audio the right volume. Normalize All Peaks to -6 db for SOTs (tracking, interviews or natural sound that you want audible) and -25 db for VOs (anything that should be quiet, in the background while a script is read on-air).
    ^ If you recorded audio using a handheld mic plugged into a camcorder, change your audio channel settings (hotkey="shift+g") to get the left audio track playing in both channels.  ^link("https://imgur.com/a/Vu9fStq","Here are screenshots of both steps.")
    `)
  }
  ...
  if (message.match(/green screen|blue screen|key|keying|ultra key|greenscreen|bluescreen|remove background|chroma-key|chroma|chromakey|chroma key|greenscreens|green screens/)) {
    response.push(`To remove a green screen, use the Ultra Key effect. It's in the effects window, under Video Effects>Keying>Ultra Key. Drag the effect onto your clip, then open the Effect Control window. Click the eye-dropper tool in the key color section, and click somewhere on the green screen in your video preview. One last thing: change the Ultra Key setting from Default to Aggressive. \n Now anything you layer behind this clip will appear where the green screen once was. ^link("https://imgur.com/a/RygkVbv","Here are some screenshots of the process.")`)
  }
  ...
  if (message.match(/placeholder|placeholders/)) {
    response.push(`Are you trying to make edit placeholders, or register a finished segment to a placeholder? ^buttonsSlack(Edit Placeholder, Register to Placeholder)`)
  }
  if (message.match(/video exporting/)) {
    response.push(`^link("http://resources.uscannenbergmedia.com/2017/09/exporting-videos-for-newscasts/","Here's how to export segments once you're done and have producer approval.")`)
  }
  ...
  if (message.match(/inews help/)) {
    response.push(`^link("https://drive.google.com/file/d/0B_ojQ9GFYg9WVnhKaUVJOVQ1eFE/view","Here's a rough guide on using iNews for ATVN.")`)
  }
  ...
  if (message.match(/settings for audio|newsboss|standalone audio/)) {
    response.push(`^link("http://resources.uscannenbergmedia.com/category/radio/","Here's a rough guide to everything audio in the media center, from recording to editing to going live on air.")`)
  }
  if (message.match(/audition|editing audio|editing audition/)) {
    response.push(`^link("https://docs.google.com/document/d/1TkXPjvmuAV6Vy6HH4NJ5xOJ-tkrM2NTekGbisIcYEks/edit","Here's an in-depth guide to working with Adobe Audition.")`)
  }
  if (message.match(/social template|downloading social template|downloading template|template not downloading|template/)) {
    response.push(`The URL for downloading all our social templates is: http://bit.ly/annenbergtemplates \n If the social template isn't downloading, try again and click "Leave" when asked if you want to leave the site. ^link("https://i.imgur.com/2O492Jr.png","Here's a screengrab showing what I'm talking about.")`)
  }
  ...
  if (message.match(/reserve a studio|reserve studio|studio reservation\studio reservations/)) {
    response.push(`Click a button below to jump straight to the form for reserving a studio. ^buttonsSlack(Studio A, Studio B, Studio C) ^link("http://resources.uscannenbergmedia.com/2016/10/studio-production-spaces/","Or here's a breakdown of all studio policies, including how to reserve studio time.")`)
  }
  if (message.match(/studio a/)) {
    response.push(`^link("http://annenbergprograms.com/MCTechnicalGuidelines_2019.pdf#studioa","Here's a link to the form for reserving Studio A.")`)
  }
  if (message.match(/studio b/)) {
    response.push(`^link("http://annenbergprograms.com/MCTechnicalGuidelines_2019.pdf#studiob","Here's a link to the form for reserving Studio B.")`)
  }
  if (message.match(/studio c/)) {
    response.push(`^link("http://annenbergprograms.com/MCTechnicalGuidelines_2019.pdf#studioc","Here's a link to the form for reserving Studio C.")`)
  }
  if (message.match(/fork|fork login/)) {
    response.push(`^link("http://resources.uscannenbergmedia.com/2016/10/media-center-storage/","Here's an overview on using FORK.")`)
  }
  if (message.match(/camera|tripod|recorder|camcorder|check out microphone/)) {
    response.push(`Do you need to check out equipment?  
    ^ {@ equipment room}`)
  }
  ...
  if (message.match(/Who do I talk to pitch a story?/)) {
    response.push(`I'm working on developing a better list of all students' roles in the media center. For now ^link("https://docs.google.com/spreadsheets/d/1583hUjAFtTHDwVvYIxYPs9w6uZ0NQuQmSNPVl4NkVu4/edit?usp=sharing","here's a Google Doc listing each day's ATVN student editors, directors and producers.")`)
  ...
  if (message.match(/How do I get reimbursed for Uber or Lyft|uber|lyft|reimbursement/)) {
    response.push(`To get reimbursed for a rideshare, first check with the day's Managing Editor to see if there is an Uber card available for checkout. \n If you've already taken the trip, you can be reimbursed by following these easy steps: \n Send an email to Rachel Gatus at rachelc1@usc.edu and cc christina.bellantoni@usc.edu In that email, provide the following information: \n \n Name: \n Personal Mailing Address: \n Purpose for Uber Ride (The slug of your story and your desk/editor): \n Then department of MC it's for: Digital/ARN/ATVN \n Copy of Uber receipt which must show the last four digits of the credit card used to pay for it. \n \n If you used your own vehicle to get to an assignment, you may be reimbursed for miles driven. \n Follow the same process as above except instead of sending an Uber receipt, send screenshots of the destinations on google maps/iPhone maps showing the mileage between locations/addresses (addresses will have to be shown and provided!) per driving trip to and from. \n \n Do you live in a dorm or apartment with an apartment number in the address? ^buttonsSlack(Yes I have an apartment number, No I do not)`)
  }
  if (message.match(/yes i have an apartment number/)) {
    response.push(`Make sure to include your apartment number with your address in your reimbursement email to Rachel and Christina. Lots of times that gets left out which can slow down the process.`)
  }
  if (message.match(/no i do not/)) {
    response.push(`Cool. I just want to check because apartment numbers getting left out of this process have caused a bunch of payments to be delayed.`)
  }
  if (message.match(/thank you|thanks|thank u|you're the best/)) {
    response.push(`- You're welcome!  ^image("https://media.giphy.com/media/kgKrO1A3JbWTK/giphy.gif")
    - You're welcome!  ^image("https://i.imgur.com/ROuLEl0.gif")
    - No, thank you!  ^image("https://i.giphy.com/media/3fjcbncA20T04/giphy.gif")
    - Anytime!  ^image("https://i.giphy.com/media/KnklaUtMatbZm/200w.gif")
    `)
  }
  if (message.match(/can I delete|delete stuff|delete off fork|delete/)) {
    response.push(`When it comes to other people's files and folders on FORK:  ^image("https://media3.giphy.com/media/hFIkt5s7MrzI4/giphy.gif")`)
  }
  if (message.match(/annenberg media spring 2019|spring 2019|spring 19/)) {
    response.push(`The Annenberg Media Center Spring 2019 class was unforgettable:  ^image("https://i.imgur.com/lyPEdMk.jpg")`)
  }
  if (message.match(/favorite song/)) {
    response.push(`I'm all about hip-hop, but for my favorite song I'd have to go with the MC5!  ^video("https://www.youtube.com/watch?v=74jS3dW0DtE")`)
  }
  if (message.match(/annenberg media spring 2020|spring 2020|spring 20/)) {
    response.push(`The Annenberg Media Center Spring 2020 team triumphed over huge challenges:  ^image("http://resources.uscannenbergmedia.com/wp-content/uploads/2020/07/Spring-2020-Wrap-Party-Zoom.jpg")`)
  }
  ...
  if (message.match(/free la times|la times free|la times for free|LA Times login|LA times subscription|free los angeles times|los angeles times free|los angeles times for free|Los Angeles Times login|Los Angeles times subscription|free lat|lat free|la times for free|lat login|lat subscription|la times sub|LA times subscription|la times/)) {
    response.push(`^link("https://checkout2.latimes.com/MTRDigitalSC?PID=6251","Here's the link to get your free year of LA Times access.")`)
  }
  ...
  if (message.match(/tech signups|tech policies|doors access|important technical guidelines and policies|technical guidelines and policies|tech guidelines and policies|tech guidelines|tech signup/)) {
    response.push(`^link("https://uscannenberg.qualtrics.com/jfe/form/SV_1zSCe3wj5w9aocd","Here's the url to complete the Media Center tech policies so you can check out equipment and get access to the media center after hours.")`)
  }
  if (message.match(/contact|usc contact|usc info|finding email|find email|email contact/)) {
    response.push(`You can use this contact list to find sources for stories about the university: ^link("https://docs.google.com/spreadsheets/d/1a1cTWByt0WUcHfrEI0im5EuyZ28PsZgjbH_3FCPvmWQ/edit#gid=0","Annenberg Media's USC contact spreadsheet is here.")`)
  }
  if (message.match(/social pitch/)) {
    response.push(`Fill this out and send it to a social team leader if you want to have a story promoted: ^link("https://docs.google.com/forms/d/e/1FAIpQLSdYIfxl3Q0I87G4CqXUi996IN6vEuY3tER8nuJIjE2z_ZAJuw/viewform", "Here's the social team's pitch form.")`)
  }
  ...
  if (response.size == 0) {
    await say(`Sorry, I don't understand. Would you please write down what you're trying to do on this form: https://bit.ly/2TVsFXh so I can learn and be more useful in the future? \n Or can you rephrase the query, or tell me if it falls into one of these categories? ^buttonsSlack(Video, Audio, Social, Other Media Center Tech)`)
  }
});
